...
class TestDerivedFeatures:
    ...
    def test_engineered_feature_count(self, featured_df):
        ...
        assert len(custom) >= 6, (
            f"Expected ≥6 engineered features, got {len(custom)}: {custom}"
        )
...
class TestDataLeakage:
    ...
    def test_no_test_index_in_train(self, cleaned_df, monkeypatch):
        ...
        assert len(overlap) == 0, (
            f"Train/test overlap detected on indices: {overlap}"
        )
    ...
    def test_stratification_preserves_class_ratio(self, cleaned_df, monkeypatch):
        ...
        assert abs(y_train.mean() - original_ratio) < 0.35, (
...
class TestPipelineStages:
    ...
    def test_validation_stage_independent(self):
        ...
        from data_validation import (
            check_accuracy, check_duplicates, check_completeness,
        )
        mock = pd.DataFrame({
            "customerID": ["C1", "C2"],
            "gender": ["Male", "Female"],
