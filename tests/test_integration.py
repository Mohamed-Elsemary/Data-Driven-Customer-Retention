...
class TestTrainingServingSkew:
    ...
    def test_single_row_preprocessing_matches_batch(self, integration_df):
        ...
        stateless_cols = [c for c in single_encoded.columns if c in batch_encoded.columns]
        for col in stateless_cols:
            ...
            if pd.api.types.is_numeric_dtype(batch_encoded[col]):
                assert batch_val == single_val, (
                    f"Skew detected in '{col}': " f"batch={batch_val}, single={single_val}"
                )
...
class TestFeatureComputationConsistency:
    ...
    def test_frequency_encoding_consistent_across_runs(self, integration_df, monkeypatch):
    ...
    def test_split_reproducibility_with_same_seed(self, integration_df, monkeypatch):
        ...
        X_train_1, X_test_1, y_train_1, y_test_1 = feature_engineering.split_and_encode(df)
        X_train_2, X_test_2, y_train_2, y_test_2 = feature_engineering.split_and_encode(df)
    ...
    def test_correlation_analysis_preserves_features(self, integration_df, monkeypatch):
