...
def add_engineered_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["_HighFriction_Payment"] = (
        (df["PaperlessBilling"] == 1) & (df["PaymentMethod_Electronic check"] == 1)
    ).astype(int)
...
def plot_feature_engineering_charts(df: pd.DataFrame) -> None:
    ...
    sns.boxplot(
        data=df,
        x="_Total_AddOns-Services",
        y="MonthlyCharges",
        hue="Churn",
        palette={0: "#5b9bd5", 1: "#e05c5c"},
        ax=ax,
    )
    ...
    sns.barplot(
        x=["Manual Payment (0)", "AutoPay (1)"],
        y=autopay_churn.values,
    ...
    plt.figure(figsize=(8, 5))
    sns.barplot(
        x=["Low/Normal Friction (0)", "High Friction (1)"],
        y=friction_churn.values,
    ...
    plt.figure(figsize=(8, 5))
    sns.barplot(
        x=["Single (0)", "Partial Family (1)", "Full Family (2)"],
        y=household_churn.values,
        palette="viridis",
    )
...
def spearman_feature_selection(df_corr: pd.DataFrame):
    ...
    sns.heatmap(
        target_corr.loc[top_features].to_frame(), annot=True, cmap="coolwarm", vmin=-1, vmax=1
    )
...
def pca_analysis(X_train, y_train, all_features):
    ...
    axes[0].axhline(5, color="red", ls="--", lw=0.8)
    ...
    for cls, label, color in [(0, "No Churn", "steelblue"), (1, "Churn", "coral")]:
        mask = y_train == cls
        plt.scatter(X_pca_2d[mask, 0], X_pca_2d[mask, 1], label=label, alpha=0.4, s=15, color=color)
    plt.xlabel(f"PC1 ({pca_2d.explained_variance_ratio_[0]*100:.1f}%)")
    plt.ylabel(f"PC2 ({pca_2d.explained_variance_ratio_[1]*100:.1f}%)")
    plt.title("2-D PCA — Class Separability")
    plt.legend()
    plt.tight_layout()
    ...
    plt.title(f"PCA Loadings — Top {n_95} Components")
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()
...
def permutation_feature_importance(X_train, X_test, y_train, y_test, all_features, selected_features):
    ...
    for label, feats in [
        ...
        scores = cross_val_score(
            rf_bench, X_train[feats], y_train, cv=skf, scoring="roc_auc", n_jobs=-1
        )
